...
$(document).ready(async function () {
    ...
    for (let i in courseStudentList) {
        let fullName = courseStudentList[i]["FirstName"] + " " + courseStudentList[i]["LastName"];
        let studentEmail = courseStudentList[i]["StudentEmail"];
        let studentId = courseStudentList[i]["StudentId"]

        let appendRow = `<tr><td>${fullName}</td><td>${studentEmail}</td><td>${studentId}</td></tr>`;

        console.log(appendRow)

        $("tbody#studentList").prepend(appendRow);
    }

    $("#deleteCourse").click(() => {
        var url = "../instructor-dash.html";
        $.get(`deleteCourse?courseId=${queryString.courseId}`);
